...
#' 
#' # Adjusting defaults
#' engagePlot(survey_data, group_column = 2, response_column = 1, NAtoZero = FALSE, totals = FALSE)
...
surveyPivot <- function(data, group_column = 1, response_column = 2, sep = ",", NAtoZero = TRUE, totals = TRUE) {
  ...
  if(length(names(data))>2) {
    ...
    stop("Warning: dataframe too wide. Ensure your dataframe contains only two variables, group and response, before continuing.")
  ...
  max_response <- max(stringr::str_count(deframe(data[,response_column]),"\\S,\\S"), na.rm = TRUE)
  ...
  if(totals==TRUE) {
    ...
    survey_pivot %<>% dplyr::arrange(desc(Totals))
